...
\end{aligned} \tag{4}
$$
可以发现，如果我们加一个约束将会极大简化这个式子，即要求 $a_t^2 + b_t^2 =1$，这样上面的平方和就等于1了。如果我们记 $\bar{a}_t = a_{t}\dots a_1$，就有
$$
\boldsymbol{x}_t = \bar{a}_t \boldsymbol{x}_0 + \sqrt{1- \bar{a}_t}\bar{\boldsymbol{\varepsilon}}_t, \quad \bar{\boldsymbol{\varepsilon}}_t \sim \mathcal{N}(\mathbf{0}, \mathbf{I}) \tag{5}
$$
...
同时还能写出式(5)的概率形式
$$
\boldsymbol{x}_t \sim q(\boldsymbol{x}_t | \boldsymbol{x}_{0})=\mathcal{N}(\boldsymbol{x}_t ; \sqrt{\bar{a}_t} \boldsymbol{x}_0 , (1- \bar{a}_t)\mathbf{I})) ,\quad \bar{a}_t=\prod_{s=1}^t \alpha_s \tag{8}
$$
整个前向过程是一个后验估计，被表示为
$$
...
$$
q\left(\boldsymbol{x}_t | \boldsymbol{x}_{t-1}, \boldsymbol{x}_0\right)=\frac{q\left(\boldsymbol{x}_{t-1}| \boldsymbol{x}_t, \boldsymbol{x}_0\right) q\left(\boldsymbol{x}_t | \boldsymbol{x}_0\right)}{q\left(\boldsymbol{x}_{t-1} | \boldsymbol{x}_0\right)}
$$
这样我们可以重写上面的优化目标
$$
\begin{aligned}
    \log p(\boldsymbol{x}_0) 
        ...
        &=\mathbb{E}_{q(\boldsymbol{x}_{1: T} | \boldsymbol{x}_0)}\left[\log \frac{p(\boldsymbol{x}_T) p_{\boldsymbol{\theta}}(\boldsymbol{x}_0 | \boldsymbol{x}_1)}{q(\boldsymbol{x}_1 | \boldsymbol{x}_0)}+\log \prod_{t=2}^T \frac{p_{\boldsymbol{\theta}}(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t)}{\frac{q(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t, \boldsymbol{x}_0) q(\boldsymbol{x}_t | \boldsymbol{x}_0)}{q(\boldsymbol{x}_{t-1} | \boldsymbol{x}_0)}}\right] \\
            &=\mathbb{E}_{q(\boldsymbol{x}_{1: T} | \boldsymbol{x}_0)}\left[\log \frac{p(\boldsymbol{x}_T) p_{\boldsymbol{\theta}}(\boldsymbol{x}_0 | \boldsymbol{x}_1)}{q(\boldsymbol{x}_1 | \boldsymbol{x}_0)}+\log \prod_{t=2}^T \frac{p_{\boldsymbol{\theta}}(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t)}{\frac{q(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t, \boldsymbol{x}_0) g(\boldsymbol{x}_{\boldsymbol{c}}+\boldsymbol{x}_0)}{q(\boldsymbol{x}_{t-1} | \boldsymbol{x}_0)}}\right] \\
...
现在我们希望进一步对 $q\left(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t, \boldsymbol{x}_0\right), p_\boldsymbol{\theta}(\boldsymbol{x}_{t-1}|\boldsymbol{x}_t)$ 进行建模，这样才能求解他们之间的KL散度。
...
\begin{aligned}
    ...
    &\propto \exp \left\{-\frac{1}{2}\left[-\frac{2 \sqrt{\alpha_t} \boldsymbol{x}_t \boldsymbol{x}_{t-1}}{1-\alpha_t}+\frac{\alpha_t \boldsymbol{x}_{t-1}^2}{1-\alpha_t}+\frac{\boldsymbol{x}_{t-1}^2}{1-\bar{\alpha}_{t-1}}-\frac{2 \sqrt{\alpha_{t-1}} \boldsymbol{x}_{t-1} \boldsymbol{x}_0}{1-\bar{\alpha}_{t-1}}\right]\right\}\\
...
因此我们建模了 $q\left(\boldsymbol{x}_{t-1} | \boldsymbol{x}_t, \boldsymbol{x}_0\right)$
$$
\boldsymbol{x}_{t-1} \sim q\left(\boldsymbol{x}_{t-1}|\boldsymbol{x}_t, \boldsymbol{x}_0\right) = \mathcal{N}(\boldsymbol{\mu}_q, \boldsymbol{\Sigma}_q)= \mathcal{N}\left(\frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1}) \boldsymbol{x}_t+\sqrt{\bar{\alpha}_{t-1}}(1-\alpha_t) \boldsymbol{x}_0}{1-\bar{\alpha}_t}, \frac{(1-\alpha_t)(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_t} \mathbf{I}\right)
$$
现在还缺 $p_\boldsymbol{\theta}(\boldsymbol{x}_{t-1}|\boldsymbol{x}_t) = \mathcal{N}(\boldsymbol{\mu}_\boldsymbol{\theta}, \boldsymbol{\Sigma}_\boldsymbol{\theta})$ 的建模，方差可以直接建模成一样的 $\boldsymbol{\Sigma}_\boldsymbol{\theta} = \boldsymbol{\Sigma}_q$，而均值与 $\boldsymbol{x}_0$ 无关，我们保持与 $\boldsymbol{\mu}_q$ 其他项都一致，只需要学 $\boldsymbol{x}_0$。因此
$$
\boldsymbol{\mu}_\boldsymbol{\theta} = \frac{\sqrt{\alpha_t}(1-\bar{\alpha}_{t-1}) \boldsymbol{x}_t+\sqrt{\bar{\alpha}_{t-1}}(1-\alpha_t) f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)}{1-\bar{\alpha}_t}
$$
根据
$$
...
\begin{aligned}
    ...
    =& \underset{\boldsymbol{\theta}}{\arg \min } \frac{1}{2 \sigma_q^2(t)}\left[\left\|\frac{\sqrt{\alpha_t}\left(1-\bar{\alpha}_{t-1}\right) \boldsymbol{x}_t+\sqrt{\bar{\alpha}_{t-1}}\left(1-\alpha_t\right) f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)}{1-\bar{\alpha}_t}-\frac{\sqrt{\alpha_t}\left(1-\bar{\alpha}_{t-1}\right) \boldsymbol{x}_t+\sqrt{\bar{\alpha}_{t-1}}\left(1-\alpha_t\right) \boldsymbol{x}_0}{1-\bar{\alpha}_t}\right\|_2^2\right]\\
    =& \underset{\boldsymbol{\theta}}{\arg \min } \frac{1}{2 \sigma_q^2(t)}\left[\left\|\frac{\sqrt{\bar{\alpha}_{t-1}}\left(1-\alpha_t\right) f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)}{1-\bar{\alpha}_t}-\frac{\sqrt{\bar{\alpha}_{t-1}}\left(1-\alpha_t\right) \boldsymbol{x}_0}{1-\bar{\alpha}_t}\right\|_2^2\right]\\
    =& \underset{\boldsymbol{\theta}}{\arg \min } \frac{1}{2 \sigma_q^2(t)}\left[\left\|\frac{\sqrt{\bar{\alpha}_{t-1}}\left(1-\alpha_t\right)}{1-\bar{\alpha}_t}\left(f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)-\boldsymbol{x}_0\right)\right\|_2^2\right]\\
    =& \underset{\boldsymbol{\theta}}{\arg \min } \frac{1}{2 \sigma_q^2(t)} \frac{\bar{\alpha}_{t-1}\left(1-\alpha_t\right)^2}{\left(1-\bar{\alpha}_t\right)^2}\left[\left\|f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)-\boldsymbol{x}_0\right\|_2^2\right]
\end{aligned}
$$

> 到这里，我们就可以得出结论，事实上我们是在学习从任意噪声还原回原图。

综上，我们最后优化的ELBO目标函数是
$$
\underset{\boldsymbol{\theta}}{\arg \min } \mathbb{E}_{t \sim U\{2, T\}}\left[\mathbb{E}_{q\left(\boldsymbol{x}_t \mid \boldsymbol{x}_0\right)}\left[\frac{1}{2 \sigma_q^2(t)} \frac{\bar{\alpha}_{t-1}\left(1-\alpha_t\right)^2}{\left(1-\bar{\alpha}_t\right)^2}\left[\left\|f_\boldsymbol{\theta}(\boldsymbol{x}_t, t)-\boldsymbol{x}_0\right\|_2^2\right]\right]\right]
$$
